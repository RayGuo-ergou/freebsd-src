...
This document provides an overview of the v0.5 release of MPTCP for FreeBSD.
...
At the time of writing, the Linux reference implementation is available from 
[3] as kernel sources, or as a pre-compiled package for Debian-based 
Linuxes. At least two commercial implementations also exist - in Citrix 
Netscaler [4] and Apple iOS [5].
...
compliant with RFC 6824. The kernel is built with optimisations and some 
...
For a list of changes since the previous list, please see change log at:
  http://caia.swin.edu.au/urp/newtcp/mptcp/tools/mptcp-changelog-v0.5.txt
...
2.1 Installation Options
This distribution contains the v0.5 implementation of Multipath TCP for 
FreeBSD. It is applied as a kernel patch against revision 285254 of 
FreeBSD-HEAD (FreeBSD 11). Instructions for acquiring the FreeBSD source and 
applying the patch are provided in 'README-Installation'.

There are two additional options available to get an MPTCP-capable host 
running:
  o  Install pre-built kernel binary
  o  Import pre-configured Virtual Machines (VMs) into VirtualBox [6].
...
3.3 Current Limitations
  ...
  o Occasional panic/KASSERT failure:
    Some KASSERT and panic conditions will occasionally occur and break the 
    system to ddb.
...
5.1 References
  ...
  [3] "MultiPath TCP - Linux Kernel implementation", Homepage, 
      http://multipath-tcp.org/, March 2013
  [4] Philip Eardley, "Survey of MPTCP Implementations"
      http://datatracker.ietf.org/doc/draft-eardley-mptcp-implementations-survey/
